...
export default class Hero extends PhysicsSprite {
  ...
  animate(): void {
    ...
    for (let texture in this.textures) {
      if (direction + this.lastTexture === texture) {
        ...
        if (this.lastTexture < 5) this.lastTexture++;
        else this.lastTexture = 1;
        return;
      }
    }
  ...
  resetAnimation(direction: string): void {
    this.lastTexture = 1;
  ...
  addTextures() {
    ...
    for (let texture in this.textures) {
      let rect = new Rectangle(
        texturesPositions[texture].x,
        texturesPositions[texture].y,
        tileset.width / 8,
        tileset.height / 5
      );
